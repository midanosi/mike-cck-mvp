 ...
 *   const code = OpenLocationCode.encode(47.365590, 8.524997);
 *
 *   Encode a location using one stage of additional refinement:
 *   const code = OpenLocationCode.encode(47.365590, 8.524997, 11);
 *
 *   Decode a full code:
 ...
 *   const shortCode = OpenLocationCode.shorten('8FVC9G8F+6X', 47.5, 8.5);
 *
 *   Recover the full code from a short code:
 *   const code = OpenLocationCode.recoverNearest('9G8F+6X', 47.4, 8.6);
 *   const code = OpenLocationCode.recoverNearest('8F+6X', 47.4, 8.6);
 */
...
function decode(code) {
  ...
  console.log(`code`, code)
  let new_code = structuredClone(code)
  console.log(`new_code`, new_code)
  // Strip out separator character (we've already established the code is
  // valid so the maximum is one), padding characters and convert to upper
  // case.
  new_code = new_code.replace(SEPARATOR_, "")
  new_code = new_code.replace(new RegExp(PADDING_CHARACTER_ + "+"), "")
  new_code = new_code.toUpperCase()
  console.log(`new_code`, new_code)
  // Decode the lat/lng pair component.
  const codeArea = decodePairs(new_code.substring(0, PAIR_CODE_LENGTH_))
  // If there is a grid refinement component, decode that.
  console.log(`codeArea`, codeArea)
  if (new_code.length <= PAIR_CODE_LENGTH_) {
    return codeArea
  }
  const gridArea = decodeGrid(new_code.substring(PAIR_CODE_LENGTH_))
  return CodeArea(
...
function shorten(code, latitude, longitude) {
  ...
  let new_code = structuredClone(code)
  if (new_code.indexOf(PADDING_CHARACTER_) != -1) {
    throw "ValueError: Cannot shorten padded codes: " + code
  }
  new_code = new_code.toUpperCase()
  const codeArea = decode(new_code)
  ...
  for (let i = PAIR_RESOLUTIONS_.length - 2; i >= 1; i--) {
    ...
    if (range < PAIR_RESOLUTIONS_[i] * 0.3) {
      // Trim it.
      return new_code.substring((i + 1) * 2)
    }
  }
  return new_code
}
 ...
 */
const clipLatitude = function(latitude) {
 ...
 */
const computeLatitudePrecision = function(codeLength) {
  if (codeLength <= 10) {
 ...
 */
const normalizeLongitude = function(longitude) {
  while (longitude < -180) {
 ...
 */
const encodePairs = function(latitude, longitude, codeLength) {
  let code = ""
 ...
 */
const encodeGrid = function(latitude, longitude, codeLength) {
  let code = ""
  ...
  let adjustedLatitude = latitude % latPlaceValue
  let adjustedLongitude = longitude % lngPlaceValue
 ...
 */
const decodePairs = function(code) {
 ...
 */
const decodePairsSequence = function(code, offset) {
  let i = 0
  let value = 0
 ...
 */
const decodeGrid = function(code) {
  let latitudeLo = 0.0
  let longitudeLo = 0.0
  let latPlaceValue = GRID_SIZE_DEGREES_
  let lngPlaceValue = GRID_SIZE_DEGREES_
...
obj.CodeArea.fn = CodeArea.prototype = {
...
obj.CodeArea.fn.init.prototype = obj.CodeArea.fn
...
function CodeArea(
    ...
    return new obj.CodeArea.fn.init(
      latitudeLo,
      longitudeLo,
      latitudeHi,
      longitudeHi,
